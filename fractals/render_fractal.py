...
class FractalRenderingApp:
    ...
    def __init__(self, window_config=None, fractal_config=None, control_config=None, output_dir=None, cmaps=None):
        ...
        self.canvas_mandel = RenderCanvas((0, 0), self.window_size, self.pix_scale, temp_range_x)
        self.canvas_mandel.init()
        self.canvas_mandel.add_framebuffer('ITER', GL_R32F, GL_RED, GL_FLOAT)
        self.canvas_mandel.add_framebuffer('COLOR', GL_RGBA, GL_RGBA, GL_UNSIGNED_BYTE)
        ...
        # TODO : Switch canvases by ID only
        # Active canvas
        self.fractal_id = 0
        self.active_canvas_name = 'MANDELBROT'
        self.active_canvas = self.canvas_mandel
    ...
    def run(self):
        while self.window_open:
            ...
            self.canvas_mandel.update_mouse_pos()
    ...
    def _action_increase_num_iter(self, action_type):
        if action_type == glfw.PRESS:
            ...
            self.fractal_config[self.active_canvas_name]['N_ITER'] = int(temp_num_iter)
    ...
    def _action_reset_view(self, action_type):
        if action_type == glfw.PRESS:
            ...
            temp_num_iter = self.fractal_config[self.active_canvas_name]['N_ITER_MIN']
            self.fractal_config[self.active_canvas_name]['N_ITER'] = int(temp_num_iter)
    ...
    def _action_switch_fractals(self, action_type):
        if action_type == glfw.PRESS:
            self.fractal_id = (self.fractal_id + 1) % 3
    ...
    def _callback_cursor_position(self, window, x_pos, y_pos):
        self.canvas_mandel.mouse_pos = (x_pos, y_pos)
    ...
    def _set_canvas_sizes(self):
        ...
        if self.fractal_id == 0:
            self.canvas_mandel.resize(self.window_size, self.pix_scale)
    ...
    def _set_canvas_settings(self):
        ...
        # Julia set
            ...
            self.canvas_mandel.reset_shift_and_scale()
    ...
    def _get_fractal_info_text(self):
        text = (
            ...
            f'ITER = {self.fractal_config[self.active_canvas_name]["N_ITER"]}\n'
    ...
    def _render_call(self):
        ...
        if self.fractal_id == 1:
            self.fractal_constant = self.canvas_mandel.s2w(self.canvas_mandel.mouse_pos)

        # Render Mandelbrot set
        if self.fractal_id == 0 or self.fractal_id == 1:
            num_iter = self.fractal_config['MANDELBROT']['N_ITER']
            self._render_fractal_program(self.canvas_mandel, self.program_mandel, self.uniforms_mandel, (0, 0), num_iter)
            self.render_texture(self.window_size, self.canvas_mandel.pos, self.canvas_mandel.size, self.canvas_mandel.framebuffer['COLOR'].tex_id)

        # Render Julia set
        if self.fractal_id == 1 or self.fractal_id == 2:
            num_iter = self.fractal_config['JULIA']['N_ITER']
            self._render_fractal_program(self.canvas_julia, self.program_julia, self.uniforms_julia, self.fractal_constant, num_iter)
